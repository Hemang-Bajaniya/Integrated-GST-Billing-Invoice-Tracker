...
export function VendorForm({ vendor, onSuccess, onCancel }: VendorFormProps) {
  ...
  const lookupPincode = useCallback(async (pin: string) => {
    ...
    try {
      ...
      if (stateAbbr && !form.getValues("state")) {
        form.setValue("state", stateAbbr, { shouldValidate: true });
      }
  ...
  const onSubmit = async (data: VendorFormData) => {
    ...
    try {
      ...
      if (vendor) {
        await vendorsService.update(vendor.id, vendorData);
      ...
      const message = error.response?.data?.message || error.message || "Failed to save vendor";
      toast.error(message);
    } finally {
      setIsLoading(false);
    }
